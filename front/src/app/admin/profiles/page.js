...
const Profiles = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axiosInstance.get(`${API_BASE_URL}/profiles/pageable?page=${currentPage}&size=${pageSize}`);
                if (res.status === 200) {
    ...
    const handleModalOpen = async (action, mode, idUnit) => {
        ...
        if (mode != "add") {
            try {
                const res = await axiosInstance.get(`${API_BASE_URL}/profiles/${idUnit}`);
                if (res.status === 200) {
                    setCurrentProfile({
                        id: res.data.id,
                        name: res.data.name,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (submitType === "delete") {
                ...
                res = await axiosInstance.post(`${API_BASE_URL}/profiles/`, currentProfile);
            } else if (submitType === "update") {
                res = await axiosInstance.put(`${API_BASE_URL}/profiles/${currentProfile.id}`, currentProfile);
            } else {
                    ...
                    <div className="modal-content">
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="modal-body">
                                ...
                                {modeModal === "delete" && (
                                    <>
                                        Deseja excluir a unidade {currentProfile.name}?
                                    </>
                                )}
                            </div>
...
export default withAdmin(Profiles);
