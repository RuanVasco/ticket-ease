...
public class TicketService {
  ...
  public Ticket create(TicketRequestDTO ticketRequestDTO, User user) {
    Form form =
        formRepository
            .findById(ticketRequestDTO.formId())
            .orElseThrow(
                () ->
    ...
    List<TicketResponse> responses =
        ticketRequestDTO.responses().stream()
            .map(
                fieldAnswer -> {
                  FormField field =
  ...
  public Page<Ticket> findPendingTicketsForApprover(User approver, Pageable pageable) {
    return ticketRepository.findPendingTicketsByApprover(
        approver.getId(), StatusEnum.PENDING_APPROVAL, pageable);
  }
  ...
  public Page<Ticket> findByOwnerOrObserverAndStatus(
      User user, StatusEnum status, Pageable pageable) {
    return ticketRepository.findByOwnerOrObserverAndStatus(user.getId(), status, pageable);
  }

  public Page<Ticket> findByDepartmentAndStatus(
      Department department, User user, StatusEnum status, Pageable pageable) {
  ...
  public void approveOrReject(Ticket ticket, boolean approved, User approver) {
    ...
    String notificationContent =
        "Ticket " + ticket.getId() + " " + (approved ? "aprovado" : "negado");
  ...
  private Page<Ticket> filterAndPaginate(
      Collection<Ticket> tickets, Department department, User user, Pageable pageable) {
    ...
    List<Ticket> pageContent =
        (start >= filteredList.size()) ? List.of() : filteredList.subList(start, end);
