...
public record FormResponseDTO(
  ...
  public static FormResponseDTO from(Form form) {
    return new FormResponseDTO(
        ...
        form.getTitle(),
        form.getDescription(),
        new TicketCategoryDTO(form.getTicketCategory()),
        UserResponseDTO.from(form.getCreator()),
        form.getApprovers().stream().map(UserResponseDTO::from).collect(Collectors.toSet()),
        form.getApprovalMode(),
        form.getFields());
  }

  public Form toEntity(
      TicketCategory category, User creatorUser, Set<User> approvers, List<FormField> formFields) {
    Form form = new Form();
    form.setId(id);
    form.setTitle(title);
