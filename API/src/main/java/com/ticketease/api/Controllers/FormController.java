...
public class FormController {
  ...
  public ResponseEntity<?> getAllPageable(Pageable pageable) {
    ...
    List<Form> filteredList =
        allForms.stream()
            .filter(
                form -> {
                  Department dept = form.getTicketCategory().getDepartment();
                  return user.hasPermission("MANAGE_FORM", dept)
  ...
  public ResponseEntity<Form> createForm(@RequestBody FormDTO formDTO) {
    ...
    TicketCategory ticketCategory =
        ticketCategoryRepository
            .findById(formDTO.getTicketCategoryId())
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.NOT_FOUND, "Ticket category not found"));
    ...
    for (FormFieldDTO dto : formDTO.getFields()) {
      ...
      if (dto.getOptions() != null) {
        List<Option> options =
            dto.getOptions().stream().map(OptionDTO::toEntity).collect(Collectors.toList());
  ...
  public ResponseEntity<Form> updateForm(@PathVariable Long formId, @RequestBody FormDTO formDTO) {
    ...
    for (FormFieldDTO dto : formDTO.getFields()) {
      ...
      if (dto.getOptions() != null) {
        field.setOptions(
            new ArrayList<>(dto.getOptions().stream().map(OptionDTO::toEntity).toList()));
      }
    }
