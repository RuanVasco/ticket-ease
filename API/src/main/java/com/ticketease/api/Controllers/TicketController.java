...
public class TicketController {
  ...
  public ResponseEntity<Page<TicketResponseDTO>> getByStatus(
      @RequestParam(required = false) StatusEnum status, Pageable pageable) {
  ...
  public ResponseEntity<?> getDepartmentTickets(
    ...
    Department department =
        departmentService
            ...
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Setor não encontrado"));
  ...
  public ResponseEntity<Page<TicketResponseDTO>> getManagedTickets(
    ...
    Page<TicketResponseDTO> resultPage =
        new PageImpl<>(
            pageContent.stream().map(TicketResponseDTO::from).toList(), pageable, filtered.size());
  ...
  public ResponseEntity<?> approveTicket(
      @PathVariable Long ticketId, @RequestParam boolean approved) {
    User user = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    Ticket ticket =
        ticketService
            .findById(ticketId)
