...
public class UserController {
  ...
  public ResponseEntity<?> getDepartments() {
    ...
    boolean allDepartmentsNull =
        user.getRoleBindings().stream().allMatch(binding -> binding.getDepartment() == null);
    ...
    List<Department> sortedDepartments =
        departments.stream().sorted(Comparator.comparing(Department::getId)).toList();
  ...
  public ResponseEntity<?> updateUser(
      @PathVariable Long userID, @RequestBody UserDTO userUpdateDTO) {
    User sender = (User) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    if (!sender.hasPermission("EDIT_USER", null)) {
      return new ResponseEntity<>(
          "Acesso negado. Você não tem permissão para editar usuários.", HttpStatus.FORBIDDEN);
    }
  ...
  public ResponseEntity<?> registerUser(@RequestBody UserDTO signUpDto) {
    ...
    if (!sender.hasPermission("CREATE_USER", null)) {
      return new ResponseEntity<>(
          "Acesso negado. Você não tem permissão para criar usuários.", HttpStatus.FORBIDDEN);
    }
